...
void append_mmio(mmio_device device) {
    if (mmio_dev_index >= MMIO_MAX_DEVICES) {
        debugf_debug("Attempt to append more devices than the maximum amount. "
                     "Quitting task...\n");
        debugf_debug("\tdevice index: %d; maximum supported devices: %d",
                     mmio_dev_index, MMIO_MAX_DEVICES);
        return;
    }
