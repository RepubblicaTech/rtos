...
// given the PML4 table and a virtual address, returns the page entry with its flags
...
void paging_init(uint64_t* kernel_pml4) {
	...
	/*
		...
		GUYS WE CAN MAP INDIVIDUAL KERNEL SECTIONS WITH PROPER PERMISSIONS AND WITHOUT PAGE FAULT AND REBOOT
		LET'S GOOOO
	*/
	...
	for (uint64_t i = 0; i < limine_parsed_data.memmap_entry_count; i++)
	{
		...
		map_region_to_page(kernel_pml4, memmap_entry->base, memmap_entry->base, memmap_entry->length, PMLE_USER_READ_WRITE);
		map_region_to_page(kernel_pml4, memmap_entry->base, PHYS_TO_VIRTUAL(memmap_entry->base), memmap_entry->length, PMLE_KERNEL_READ_WRITE);
	}
