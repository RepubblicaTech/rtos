...
void pmm_init() {
	...
	{
		struct limine_memmap_entry *memmap_entry = limine_data.limine_memory_map[i];
...
int get_freelist_entry_count() {
	...
	for (usable_entry_count = 0; fl_en != NULL; usable_entry_count++, fl_en = fl_en->next);
...
void *fl_alloc(size_t bytes) {
	...
	for (fl_entry = fl_head; fl_entry != NULL; fl_entry = fl_entry->next)
	{
		...
		if (bytes <= fl_entry->length) {
			...
			fl_entry->length -= bytes;		// subract the allocated size from entry's length
...
void fl_free(void *ptr) {
	...
	freelist_node *fl_en;
	for (fl_en = fl_head; fl_en->next != NULL; fl_en = fl_en->next) last_fl = fl_en;
