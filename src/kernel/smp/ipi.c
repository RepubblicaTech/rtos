...
extern void ipi_handler_reschedule(registers_t *regs);
extern void ipi_handler_test(registers_t *regs);
extern void do_nothing_and_shut_up_im_talking_to_you_vector_254_yes_you_just_dont_spam_logs_ok_thanks(registers_t *regs);
...
void register_ipi(void) {
    ...
    isr_registerHandler(254, do_nothing_and_shut_up_im_talking_to_you_vector_254_yes_you_just_dont_spam_logs_ok_thanks); // * note: this is not malware. Context: the logs would always be full with "Unhandled interrupt 254"
}
...
void ipi_send(uint8_t vector, uint8_t cpu) {
   ...
   // send le IPI
    uint32_t icr0 = vector | (0 << 8) | (0 << 11) | (1 << 14) | (0 << 15) | (LDT_NO_SHORTHAND << 18);
    lapic_write_reg(LAPIC_ICR0_REG, icr0);
}

void ipi_broadcast(uint8_t vector) {
    // broadcast and ipi to all other LAPICs except our
    uint32_t icr0 = vector | (0 << 8) | (0 << 11) | (1 << 14) | (0 << 15) | (LDT_ALL_LAPICS_NOT_US << 18);
    lapic_write_reg(LAPIC_ICR0_REG, icr0);
}
