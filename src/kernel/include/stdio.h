...
#define kprintf_ok(fmt, ...)                                                          ({\
    ...
    kprintf("[ %s():%d::SUCCESS ] " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__);        \
...
#define kprintf_info(fmt, ...)                                                        ({\
    ...
    kprintf("[ %s():%d::INFO ] " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__);           \
    set_screen_fg(prev_fg);                                                             \
})
...
#define kprintf_warn(fmt, ...) ({                                                       \
    ...
    kprintf("--- [ WARNING @ s():%d ] --- " fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__);\
    set_screen_fg(prev_fg);                                                             \
})
...
#ifdef BETTER_DEBUG
    #define debugf_debug(fmt, ...)  debugf("[ %s()::DEBUG ] "fmt, __FUNCTION__, ##__VA_ARGS__)
#endif
