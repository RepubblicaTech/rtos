...
void pic_sendEOI(uint8_t irq) {
	if(irq >= 8) _outb(PIC2_COMMAND, PIC_EOI);
...
void pic_config(int offset1, int offset2) {
	...
	_outb(PIC1_COMMAND, PIC_ICW1_INITIALIZE | PIC_ICW1_ICW4);    // starts the initialization sequence (in cascade mode)
	_io_wait();
	...
	_outb(PIC1_DATA, 0x04);                                      // ICW3: tell Master PIC that there is a slave PIC at IRQ2 (0000 0100)
	_io_wait();
	_outb(PIC2_DATA, 0x02);                                      // ICW3: tell Slave PIC its cascade identity (0000 0010)
	_io_wait();
	
	_outb(PIC1_DATA, PIC_ICW4_8086);                             // ICW4: have the PICs use 8086 mode (and not 8080 mode)
	_io_wait();
